...
#define APP_SIGN_LED_PORT       GPIOG
#define APP_SIGN_LED_PIN        GPIO_PIN_10
#define APP_SIGN_LED_PULSE_MS   140U
    ...
    while (1)
    {
        ...
        if (is_tracking != 0)
        {
            ...
            if (is_tracking != 0)
            {
                ...
                {
                    app_decode_ld_landmarks(&app_pd_rois[0], &ld_landmarks[0][j], &gesture_landmarks[j]);
                }
                sign_result = app_sign_update(&sign_state, gesture_landmarks, HAL_GetTick());
                if (sign_result.confirmed != 0U)
                {
                    app_emit_sign_log(&sign_result, HAL_GetTick());
                    app_sign_success_feedback(HAL_GetTick());
                }
            }
        ...
        else
        {
            ...
            sign_result = (app_sign_result_t){0};
            sign_result.phrase = sign_state.last_phrase;
        }
        ld_filtered_ms = (7 * ld_filtered_ms + hl_ms) / 8;
        app_sign_feedback_poll(HAL_GetTick());
...
static void app_display_network_output(app_display_info_t *display_info)
{
    ...
    line_nb += 2;
    UTIL_LCDEx_PrintfAt(0, LINE(line_nb), RIGHT_MODE, "Sign");
    line_nb += 1;
    UTIL_LCDEx_PrintfAt(0, LINE(line_nb), RIGHT_MODE, "Cand %s",
                        app_sign_phrase_id(display_info->sign.candidate));
    line_nb += 1;
    UTIL_LCDEx_PrintfAt(0, LINE(line_nb), RIGHT_MODE, "%s",
                        app_sign_gesture_name(display_info->sign.candidate));
    line_nb += 1;
    UTIL_LCDEx_PrintfAt(0, LINE(line_nb), RIGHT_MODE, "Hold %3u%%",
                        display_info->sign.progress);
    line_nb += 1;
    UTIL_LCDEx_PrintfAt(0, LINE(line_nb), RIGHT_MODE, "%lu/%lums",
                        (unsigned long)display_info->sign.hold_ms,
                        (unsigned long)app_sign_confirm_hold_ms());
    line_nb += 2;
    UTIL_LCDEx_PrintfAt(0, LINE(line_nb), RIGHT_MODE, "Result");
    line_nb += 1;
    UTIL_LCDEx_PrintfAt(0, LINE(line_nb), RIGHT_MODE, "%s %s",
                        app_sign_phrase_id(display_info->sign.phrase),
...
static void app_emit_sign_log(app_sign_result_t *sign, uint32_t now_ms)
{
    ...
    len = snprintf(line, sizeof(line),
                   "{\"t\":%lu,\"ver\":1002,\"event\":\"sign_confirmed\",\"id\":\"%s\",\"phrase\":\"%s\",\"gesture\":\"%s\",\"hold_ms\":%lu,\"cooldown_ms\":%lu}\r\n",
                   (unsigned long)now_ms,
                   app_sign_phrase_id(sign->phrase),
                   app_sign_phrase_text(sign->phrase),
                   app_sign_gesture_name(sign->phrase),
                   (unsigned long)sign->hold_ms,
                   (unsigned long)app_sign_repeat_cooldown_ms());
...
static void app_sign_success_feedback(uint32_t now_ms)
{
    HAL_GPIO_WritePin(APP_SIGN_LED_PORT, APP_SIGN_LED_PIN, GPIO_PIN_RESET);
    sign_led_off_ms = now_ms + APP_SIGN_LED_PULSE_MS;
}

static void app_sign_feedback_poll(uint32_t now_ms)
{
    if ((sign_led_off_ms != 0U) && ((int32_t)(now_ms - sign_led_off_ms) >= 0))
    {
        HAL_GPIO_WritePin(APP_SIGN_LED_PORT, APP_SIGN_LED_PIN, GPIO_PIN_SET);
        sign_led_off_ms = 0U;
    }
}
